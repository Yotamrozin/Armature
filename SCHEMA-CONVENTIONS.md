...
- **Hide what's irrelevant.** When a field only matters conditional on another field's value, hide it (`hidden: ({ parent }) => ...`) instead of leaving it always visible. See `objects/link.ts`'s `internal`/`external` toggle — showing both regardless of `linkType` would just be confusing. Say so in the field's own `description` (e.g. "Only shown when Link type above is set to..."), so a field appearing/disappearing when its trigger changes isn't a surprise — a permanent placeholder row explaining the same thing would work against "hide what's irrelevant" by keeping the form the same length either way.
